...
export default function BehindScenesFeed({ posts, currentUserId, currentUserName }: { posts: BehindScenesFeedPost[]; currentUserId: string; currentUserName: string | null }) {
  ...
  return (
    <div style={{ marginTop: isMobile ? 20 : 24, marginBottom: 6 }}>
      ...
      {isMobile ? (
        // على الجوال: شريط أفقي مضغوط (أصغر من بطاقات المشاريع) بمنشورات
        // فعلية، يُتصفَّح بسحب يمين/يسار بدل التمرير العمودي — يحافظ على
        // إطار الصفحة العام بدل مساحة كبيرة رأسية.
...
// بطاقة منشور مضغوطة لشريط الجوال الأفقي — أصغر من بطاقة المشروع عمداً،
// عرض ثابت ضمن شريط سحب أفقي بدل بطاقة كاملة العرض.
    ...
    >
      {cover && (
        <div style={{ position: "relative", width: "100%", height: 96, background: "#000" }}>
          ...
          {post.media.length > 1 && (
            <span style={{ position: "absolute", top: 6, insetInlineEnd: 6, background: "rgba(0,0,0,0.6)", color: "#fff", fontSize: 9.5, fontWeight: 700, borderRadius: 7, padding: "2px 6px" }}>
              +{post.media.length - 1}
            </span>
          )}
        </div>
      ...
      <div style={{ padding: 8 }}>
        ...
        {post.title && (
          <div style={{ fontSize: 11.5, fontWeight: 700, marginTop: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{post.title}</div>
        )}
        <div style={{ fontSize: 9.5, color: "var(--text-muted)", marginTop: 2 }}>{relativeTime(post.createdAt)}</div>
      </div>
    </button>
  );
}
