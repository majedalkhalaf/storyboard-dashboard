...
export default function ClientsStatsRow({ stats }: { stats: ClientsDirectoryStats }) {
  const cards: { label: string; value: string; icon: IconName; color: string; sub?: string; progress?: number }[] = [
    ...
    { label: "إجمالي قيمة العقود", value: `${stats.totalContractsValue.toLocaleString("en-US")} ر.س`, icon: "contracts", color: "#CE902F" },
    { label: "إجمالي الفواتير", value: `${stats.totalInvoices.toLocaleString("en-US")} ر.س`, icon: "invoices", color: "#06B6D4" },
    { label: "إجمالي المدفوعات", value: `${stats.totalPaid.toLocaleString("en-US")} ر.س`, icon: "payments", color: "#22C55E" },
    { label: "المبالغ المتبقية", value: `${stats.totalRemaining.toLocaleString("en-US")} ر.س`, icon: "money", color: "#EF4444" },
    {
