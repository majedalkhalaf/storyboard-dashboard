...
export default function BookletSectionsPanel({
  data,
  sections,
  onChange,
}: {
  ...
  const merged: PresentationSectionConfig[] = [
    ...sections.filter((s) => BOOKLET_SECTIONS.some((def) => def.key === s.key)),
    ...BOOKLET_SECTIONS.filter((def) => !known.has(def.key)).map((def) => ({ key: def.key, enabled: def.isAvailable(data) })),
  ];
  ...
  return (
    <div>
      <p style={{ fontSize: 13, color: "var(--text-muted)", marginBottom: 16 }}>
        فعّل/عطّل أي قسم، ورتّبها بالسحب والإفلات. الأقسام التي لا تملك بيانات فعلية تُعطَّل تلقائياً افتراضياً — يمكنك
        تفعيلها يدوياً وستظهر فارغة إن لم توجد بيانات.
      </p>
      ...
      <DndContext sensors={sensors} onDragEnd={handleDragEnd}>
        <SortableContext items={merged.map((s) => s.key)} strategy={verticalListSortingStrategy}>
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {merged.map((s) => {
              const def = BOOKLET_SECTIONS.find((d) => d.key === s.key);
              if (!def) return null;
