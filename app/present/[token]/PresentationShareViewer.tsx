...
// عارض تفاعلي بسيط (تالي/سابق) لصفحة المشاركة العامة — لا علاقة له بجلسة تسجيل الدخول
// إطلاقاً، ويستخدم SectionRenderer نفسه المستخدَم في المعاينة الداخلية والطباعة.
export default function PresentationShareViewer({
  data,
  texts,
  theme,
  ordered,
  ...
  if (ordered.length === 0) {
    return (
      ...
      >
        <p style={{ fontSize: 14 }}>لا يوجد محتوى متاح لهذا العرض حالياً.</p>
      </div>
    );
  }
    ...
    <div style={{ minHeight: "100vh", background: theme.bg, display: "flex", flexDirection: "column" }}>
      <div style={{ flex: 1, position: "relative", minHeight: 0 }}>
        <div style={{ position: "absolute", inset: 0 }}>
          <SectionRenderer sectionKey={currentKey} data={data} texts={texts} theme={theme} />
        </div>
